...
export default class Chat extends Component {
  ...
  componentDidMount() {
    ...
    API.post("/auth-token", token).then((res) => {
      this.setState({ userId: res.data.userId });
      API.get(`/get-chat-box?userId=${res.data.userId}`).then((res) => {
        this.setState({
  ...
  handleClickFriend = (e) => {
    ...
    API.get(`/add-chat-box?userId=${this.state.userId}&friendId=${e.currentTarget.id}`).then((res) => {
  ...
  handleExit = (e) => {
    ...
    API.get(`/delete-chat-room?chatId=${e.currentTarget.id}`).then((res) => {
  ...
  renderRoom = () => {
    return this.state.chatroom.map((datas, index) => {
      return (
    ...
    return (
      <div className="Chat">
              ...
              <li key={datas.userId} className="list-group-item">
                <LazyLoad offsetHorizontal={300}>
                  <button className="friend-list-box" id={datas.userId} onClick={this.handleClickFriend}>
                    <LazyLoadImage className="comment-picture" effect="blur" src={datas.profileurl} />
        ...
        <div className="chat-box-area">
          <div className="row">{this.renderRoom()}</div>
        </div>
      </div>
    );
  }
